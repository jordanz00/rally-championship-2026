...
check(
  "title side panes follow aperture hull, not AABB planes",
  /convexHull2/.test(celica) &&
    /tessellateAperturePane/.test(celica) &&
    !/PlaneGeometry\(ax\.w/.test(celica)
);
...
check("title Celica LOD is HTML-preloaded", /assets\/celica\/rival\.glb/.test(index) && /rel="preload"/.test(index));
