...
check("deep mud/sand caps", /mud:\s*0\.21/.test(deform) && /sand:\s*0\.165/.test(deform));
check("fine deform cell", /this\.cell\s*=\s*0\.2\b/.test(deform));
...
check("dual-layer trail (center + lip)", /profile\.center/.test(effects) && /profile\.lip/.test(effects) && /width \* 0\.68/.test(effects));
check("soft trails stamp wheelDeform + wheelRuts", /wheelDeform\.stampSegment/.test(effects) && /wheelRuts\.writeSegment/.test(effects));
check("track imports surface-deform v7+", /surface-deform\.js\?v=([7-9]|\d{2,})/.test(track));
check("mark grit shader", /vWorldXZ/.test(effects) && /grit/.test(effects));
