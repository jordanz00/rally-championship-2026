...
check("powerSlidePitch", /powerSlidePitch:\s*1\.35/.test(config), "throttle pitch-in");
check("maxSlideVel raised", /maxSlideVel:\s*17\.2/.test(config), "slide ceiling");
check("hb yaw kick strong", /handbrakeYawKick:\s*3\.15/.test(config), "initiation snap");
check("hb power mul strong", /handbrakePowerMul:\s*2\.05/.test(config), "power oversteer");
check("hb bleed low", /handbrakeBleedMul:\s*0\.032/.test(config), "long e-brake carry");
check("driftBleedMul low", /driftBleedMul:\s*0\.048/.test(config), "throttle carry");
check("slideGripMul slippery", /slideGripMul:\s*0\.26/.test(config), "angle grip");
check("sand loose", /driftEase:\s*1\.36/.test(config), "sand pitch-in");
check("gravel brakeYaw", /brakeYaw:\s*0\.58/.test(config), "gravel brake-to-slide");
