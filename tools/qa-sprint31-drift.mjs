...
check("expert trail-brake yaw", /trailBrakeYaw:\s*0\.44/.test(config));
check("expert countersteer mul", /expertCounterMul:\s*1\.18/.test(config));
...
check("counterAuthority", /counterAuthority:\s*2\.85/.test(config));
check("power slide sustain", /driftBleedMul:\s*0\.048/.test(config));
check("e-brake carry", /handbrakeBleedMul:\s*0\.032/.test(config));
check("arcade slide ceiling", /maxSlideVel:\s*17\.2/.test(config));
