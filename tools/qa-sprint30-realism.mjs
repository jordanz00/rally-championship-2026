...
check("worldEnv lifted", /worldEnvIntensity:\s*(0\.[89]\d*|1\.\d+)/.test(config), "worldEnv ≥ 0.8");
check("photographic grade sat", /gradeSaturation:\s*1\.0[0-9]/.test(config), "restrained saturation");
check("film grain off (sky grain)", /filmGrain:\s*0\s*,/.test(config), "filmGrain: 0 — grain read as sky static");
