...
check("fluffy Worley cores (not smoke sheet)", /worleyPuff\(/.test(sky) && /pow\(core/.test(sky) && /WEATHER_CONTRAST = 2\.85/.test(sky));
check("taller cloud shell", /CLOUD_OUTER = 9\.55/.test(sky));
...
check("deeper desert zenith blue", /skyZenith:\s*0x0a3e9a/.test(config) && /sunBloom:\s*1\.18/.test(config));
check("sky.js?v=32+", Number((game.match(/sky\.js\?v=(\d+)/) || [])[1]) >= 32);
check("config.js?v=160+", Number((game.match(/config\.js\?v=(\d+)/) || [])[1]) >= 160);
check("boot cache ?v=520+", Number((index.match(/main\.js\?v=(\d+)/) || [])[1]) >= 520);
