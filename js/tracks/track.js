...
export class Track {
  ...
  _buildSpline(pieces, def) {
    ...
    // Desert finale underpass + sweep flats must exist before road skirts and
    // the land plane — same failure mode as the tunnel sand slabs.
    this._underpassRuns = [];
    ...
    // Forest finale + full Mountain corridor — land stays a floor so the car
  ...
  /**
   * Same as _addLandPlane but yields one frame per tile row for progress UI.
   * @param {object} def
   * @param {object} b
  ...
  _addLandTile(def, scenery, desert, b, cx, cz, tileSize, segs, floorHex) {
    ...
    const bedDrop =
      scenery === "mountain" ? 0.78 : scenery === "forest" ? 0.72 : desert ? 0.72 : 0.28;

    for (let i = 0; i < pos.count; i++) {
      const x = pos.getX(i) + cx;
      const z = pos.getZ(i) + cz;
      const near = this._nearestRoad(x, z);
      let h = this._groundHeight(x, z, scenery);
      if (desert && this._inUnderpassCorridor(x, z)) {
        // Keep land as a floor under the arch — never a dune slab through the car.
        h = near.roadY - 0.95;
      } else if (this._inLandmarkFlat(near.along, near.dist, near.roadW)) {
        h = near.roadY - bedDrop;
      } else if (near.dist < this._trenchWidth(near.roadW)) {
        if (desert) h = near.roadY - 0.72;
        else if (scenery === "mountain") h = near.roadY - 0.78;
        else if (scenery === "forest") h = near.roadY - 0.72;
        ...
        if (near.dist < chase) {
          const bed = near.roadY - 0.72;
          h = Math.min(h, bed + Math.max(0, near.dist - this._trenchWidth(near.roadW)) * 0.03);
        }
      }
      // Absolute refuse: land polys must never sit above the ribbon deck in-lane.
      if (near.dist < near.roadW * 0.5 + ROAD_VERGE + 1.2) {
   ...
   */
  _groundHeight(x, z, scenery) {
    const { dist, roadY, roadW, tunnel, side, along } = this._nearestRoad(x, z);
    ...
    if (desert) {
      ...
      // ribbon — "heavy sand on the roadway." Keep both ends of any crossing
      // triangle at bed height; dunes still rise immediately after that.
      const hill = this._tunnelHill(along || 0, dist, roadW);
      const trench = Math.max(this._trenchWidth(roadW), roadW * 0.5 + 16);
      const rise = hill > 1 ? 18 : 26;
      const bed = roadY - 0.72;
      ...
      const washed = flatBed(bed);
      if (washed != null) return washed;
      if (this._inUnderpassCorridor(x, z)) return roadY - 0.95;
      if (dist < trench) return bed;
      if (dist < trench + rise) return bed + bank * sm((dist - trench) / rise);
      if (dist < trench + rise + blend) {
        const t = sm((dist - trench - rise) / blend);
  ...
  _addBackdropBiome(def, b) {
    ...
    if (scenery === "desert") {
      this._addBackdropTreeRings(
        b,
        [
          { kind: "acacia", r: b.maxR + 360, n: 28, hMin: 14, hMax: 24, wMin: 10, wMax: 18 },
          { kind: "acacia", r: b.maxR + 560, n: 20, hMin: 18, hMax: 30, wMin: 12, wMax: 20 },
        ],
        "desert"
      );
  ...
  _addBackdropCactusRings(b) {
    const stems = [];
    const rings = [
      { r: b.maxR + 320, n: 32, hMin: 7, hMax: 12 },
      { r: b.maxR + 560, n: 24, hMin: 10, hMax: 18 },
    ];
    for (let ri = 0; ri < rings.length; ri++) {
      const ring = rings[ri];
      for (let i = 0; i < ring.n; i++) {
        const ang = (i / ring.n) * Math.PI * 2 + (i % 3) * 0.08;
        const rad = ring.r + (((i * 9) % 7) - 3) * 12;
    ...
    const tall = stems.filter((p) => p.kind === "cactus_tall");
    const short = stems.filter((p) => p.kind === "cactus_short");
    this._addHdBackdrop("cactus_tall", tall, b);
    this._addHdBackdrop("cactus_short", short, b);
  }
  ...
  _buildMesh(def) {
     ...
     */
    const sl = desert ? 8.2 : scenery === "mountain" ? 11 : scenery === "lakeside" ? 12.5 : 12;
    const kerbW = 0.42;
    ...
    for (let i = 0; i < pts.length - 1; i++) {
      ...
      const skirtReach =
        inTunnel || inUnderpass ? 0.85 : atJump ? 1.4 : atLandmark ? 2.1 : sl;
      ...
      const skirtDrop = (edgeY, lx, lz) => {
        ...
        return this._groundHeight(lx, lz, scenery);
      };
  ...
  _addHdNature(glbKind, poses, opts) {
    ...
    if (forestParts) {
      this._addInstances(forestParts.trunk, forestParts.trunkMat, poses, opts || { castShadow: true });
      this._addInstances(forestParts.canopy, forestParts.canopyMat, poses, opts || { castShadow: true });
      return;
    }
  ...
  _addHdBackdrop(glbKind, poses, origin) {
    ...
    const forestParts = propForestTreeParts(glbKind);
    if (forestParts) {
      this._addBackdropInstances(forestParts.trunk, forestParts.trunkMat, poses, origin);
      this._addBackdropInstances(forestParts.canopy, forestParts.canopyMat, poses, origin);
      return;
    }
  ...
  async _addSceneryBody(def, onProgress) {
    ...
    const glbTree = useGlb && !!propGeometry("tree_cone");
    ...
    /** Flat-topped savannah canopy for the Safari gallery. */
    const SAFARI_KINDS = ["acacia"];
    ...
    for (let i = 2; i < this.points.length; i += plantStep) {
      ...
      for (const side of [-1, 1]) {
        ...
        if (def.scenery === "desert") {
          const pick = rng();
          if (pick < 0.4) {
            if (!this._ribbonClear(px, pz, 1.1)) continue;
            ...
            this._bumpNearRoad(px, pz, 0.55);
          } else if (pick < 0.72) {
            const s = 0.9 + rng() * 1.8;
            if (!this._ribbonClear(px, pz, s * 0.65)) continue;
            rocks.push({
              c: chunk,
              x: px,
            ...
            this._bumpNearRoad(px, pz, Math.max(0.65, s * 0.5));
          } else if (pick < 0.88) {
            ...
            // A handful of acacia on the near shoulder. Rare on purpose: the
            // Desert stage sells itself on emptiness, so a tree has to be an
            // event rather than a hedge.
            if (!this._ribbonClear(px, pz, 2.8)) continue;
            this._plantForestTree(cardBags, px, py, pz, rng, false, SAFARI_KINDS, chunk);
          }
    ...
    for (let ti = 0; ti < FOREST_TREE_KINDS.length; ti++) {
      ...
      stripDrive(bag, forest || mountain ? FOREST_TREE_CLEAR + 0.6 : 3.4);
      ...
      this._addInstances(forestParts.trunk, forestParts.trunkMat, bag, { castShadow: true });
      this._addInstances(forestParts.canopy, forestParts.canopyMat, bag, { castShadow: true });
    }
    const cards = [
      ["pine", pine],
      ["cedar", cedar],
    ...
    for (const [kind, bag] of cards) {
      stripDrive(bag, forest ? FOREST_TREE_CLEAR : 3.4);
      if (!bag.length) continue;
      ...
      if (!glbKey) continue;
      const forestParts = propForestTreeParts(glbKey);
      if (forestParts) {
        for (let i = 0; i < bag.length; i++) {
          const p = bag[i];
          if (p && p.y != null && p.sy == null) p.y = p.groundY != null ? p.groundY : p.y;
      ...
      const glbTreeGeo = this._propGeo(glbKey);
      if (!glbTreeGeo) {
        console.warn(`[scenery] missing HD tree ${glbKey} — no primitive fallback`);
        continue;
      }
      for (let i = 0; i < bag.length; i++) {
        const p = bag[i];
    ...
    if (!forest && !mountain && !lakeside) {
      if (glbTree) this._addHdNature("tree_cone", crowns, { castShadow: true });
    }
    if (glbRock && rockGeo) {
    ...
    // wall along the verge and blocked legitimate off-road runoff.
    if (def.barriers) {
      ...
      for (let i = 0; i < this.points.length; i += 3) {
        ...
        for (const side of [-1, 1]) {
          if (!this._mayPlant(p.dist, side, p.width * 0.5 + 0.6, 1)) continue;
          const bx = p.x + p.nx * side * (p.width * 0.5 + 0.6);
          const bz = p.z + p.nz * side * (p.width * 0.5 + 0.6);
  ...
  _fillWild(def, rng, bags) {
    ...
    scatter(farRocks, 16, scenery === "mountain" ? 130 : 110, (x, y, z, r, c) => {
      ...
      this._bumpNearRoad(x, z, Math.max(0.75, s * 0.55));
    });
  ...
  _plantForestTree(bags, x, y, z, rng, near, kinds, chunk, scenery) {
    const foot = scenery === "forest" || scenery === "mountain" ? FOREST_TREE_CLEAR : 3.4;
   ...
   * Stage 2 finale (sweep + linked mud hairpins) and Stage 3 full corridor.
   * @param {object} def
   */
  ...
  /**
   * Desert — flatten land + suppress road skirts before mesh/terrain build.
   * Sweep corridor (Act 6) and finale rock bridge (Act 7 approach).
   */
  _markDesertUnderpassCorridors() {
    ...
    for (let i = 0; i < this.points.length; i++) {
      ...
      if (runStart >= 0 && (!marked || last)) {
        ...
        if (end >= runStart) {
          const a = this.points[runStart];
          const b = this.points[end];
          const lateral = Math.max(a.width, b.width) * 0.5 + 52;
          this._landmarkFlats.push({ dist0: a.dist - 52, dist1: b.dist + 48, lateral });
          this._underpassRuns.push({ dist0: a.dist - 52, dist1: b.dist + 48, lateral });
          for (let j = runStart; j <= end; j++) this.points[j].underpass = true;
        }
        runStart = -1;
      }
    }
    ...
    // Match drive-through portal + abutments (see _addDesertRockBridge).
    // One land cell ≈ 10.7 m — keep every triangle that can cross the arch
    // fully at bed height (pad beyond road + abutment thickness).
    const portalHalfW = p.width * 0.5 + 2.8;
    const portalHalfD = 12;
    const lateral = Math.max(portalHalfW + 52, p.width * 0.5 + 78, this._trenchWidth(p.width) + 32);
    const dist0 = pin.dist - (portalHalfD + 60);
    const dist1 = pin.dist + (portalHalfD + 68);
    this._landmarkFlats.push({ dist0, dist1, lateral });
    this._underpassRuns.push({ dist0, dist1, lateral });
    // Oriented wash under the arch — survives hairpin nearest-road ambiguity.
    const fx = Math.sin(p.heading);
    const fz = Math.cos(p.heading);
  ...
  _registerBridgePortalPrism(p, clear) {
    ...
    this._underpassPrisms.push({
      ...
      back: clear.halfD + 28,
      fwd: clear.halfD + 36,
      halfLat: Math.max(clear.halfW + 36, p.width * 0.5 + 48),
      openH: clear.openH,
    });
  }
  ...
  _addDesertHeroLandmark() {
    ...
    if (!pin) return;
    const p = this.points[pin.i];
    const rock = new THREE.MeshLambertMaterial({ color: 0xb89a72, flatShading: true });
    const rockDark = new THREE.MeshLambertMaterial({ color: 0x7a6348, flatShading: true });
    const rockLit = new THREE.MeshLambertMaterial({ color: 0xcbb892, flatShading: true });
    this._addDesertRockBridge(p, rock, rockDark, rockLit);
  }

  /**
   * Sandstone underpass with an explicit empty portal the car drives through.
   * Nothing is placed inside |x| < clearHalfW && |z| < clearHalfD && y < openH.
  ...
  _addDesertRockBridge(p, rock, rockDark, rockLit = rock) {
    ...
    const half = p.width * 0.5;
    // Clearance prism — wide/tall enough that chase-cam and car body fit.
    const openH = 10.2;
    const clearHalfW = half + 2.8;
    const clearHalfD = 12;
    this._registerBridgePortalPrism(p, { halfW: clearHalfW, halfD: clearHalfD, openH });

    const pierInset = clearHalfW + 1.6;
     ...
     * @param {number} [rz]
     */
    const addBlock = (sx, sy, sz, x, y, z, mat, ry = 0, rx = 0, rz = 0) => {
      // Refuse any block that invades the drive-through portal volume.
      const x0 = x - sx * 0.5;
      const x1 = x + sx * 0.5;
      const y0 = y - sy * 0.5;
      const y1 = y + sy * 0.5;
      const z0 = z - sz * 0.5;
      const z1 = z + sz * 0.5;
      const overlapsPortalX = x0 < clearHalfW && x1 > -clearHalfW;
      const overlapsPortalZ = z0 < clearHalfD && z1 > -clearHalfD;
      const overlapsPortalY = y0 < openH && y1 > 0.05;
    ...
    // Left / right piers — sit fully outside the kerb clearance.
    for (const side of [-1, 1]) {
      const pierX = side * (pierInset + 3.8);
      addBlock(7.2, openH + 0.8, clearHalfD * 1.55, pierX, (openH + 0.8) * 0.5, 0, rockDark);
      addBlock(6.0, openH + 2.6, clearHalfD * 1.15, pierX + side * 5.2, (openH + 2.6) * 0.5 + 0.6, 0, rock);
      // Outer buttress (further out, does not face into the lane).
      addBlock(8.5, openH + 4.5, 10, pierX + side * 10.5, (openH + 4.5) * 0.5 + 1.2, clearHalfD * 0.35, rockDark);
      // Readable inner face — outside clearHalfW.
      addBlock(1.4, openH - 0.35, clearHalfD * 1.7, side * (clearHalfW + 0.95), openH * 0.5 - 0.1, 0, rockLit);
      // Footing blocks — axis-aligned so they cannot rotate into the portal.
      for (let k = 0; k < 4; k++) {
        const s = 1.5 + (k % 3) * 0.5;
        addBlock(
          s * 1.5,
          s * 0.9,
          s * 1.2,
          side * (clearHalfW + 4.2 + k * 1.25),
          s * 0.45,
          -clearHalfD * 0.55 + k * 2.8,
          k % 2 ? rock : rockDark
        );
      }
    }

    // Deck / lintel — entirely above openH (underside is the portal ceiling).
    const spanW = clearHalfW * 2 + 4.5;
    addBlock(spanW, 1.7, clearHalfD * 2.05, 0, openH + 0.85, 0, rockLit);
    addBlock(spanW + 2.8, 1.2, clearHalfD * 1.65, 0, openH + 2.0, 0.3, rock);
    // Ceiling ribs (still above openH).
    for (const zOff of [-5.5, -1.8, 1.8, 5.5]) {
      addBlock(spanW - 0.8, 0.38, 0.65, 0, openH + 0.22, zOff, rockDark);
    }
    addBlock(4.5, 2.0, 5.0, 0, openH + 3.4, 0.4, rock);

    // Overburden — ONLY beyond the exit face and above the deck (never seals the mouth).
    addBlock(spanW + 12, 6.5, 9, 0, openH + 5.5, clearHalfD + 10.5, rock);
    addBlock(spanW + 20, 10, 12, 0, openH + 8.0, clearHalfD + 20, rockDark);
    for (const side of [-1, 1]) {
      addBlock(10, 14, 14, side * (half + 24), openH + 4.5, clearHalfD + 15, rockDark, 0, 0, side * 0.06);
    }

    // Crown rubble on top of the deck only — refuse if any corner dips into the portal.
    for (let i = 0; i < 7; i++) {
      const s = 1.6 + (i % 4) * 0.45;
      const x = (i - 3) * 2.6;
      const y = openH + 3.2 + (i % 3) * 0.45;
      const z = (i % 2) * 1.8 - 0.4;
      // Approximate AABB refuse (shards are irregular; stay above openH + margin).
      if (y - s * 0.7 < openH + 0.35) continue;
      if (Math.abs(x) < clearHalfW - 1.5 && Math.abs(z) < clearHalfD - 1.5 && y - s < openH) continue;
      const m = new THREE.Mesh(shard, i % 2 ? rockLit : rock);
      m.scale.set(s * 2.0, s * 1.2, s * 1.6);
      m.position.set(x, y, z);
      m.rotation.set(i * 0.25, i * 0.8, i * 0.12);
      m.castShadow = true;
      g.add(m);
    }
    ...
    g.traverse((obj) => {
      if (obj.isMesh) {
        ...
        obj.userData.desertBridge = true;
      }
    });
    this.group.add(g);

    // Keep props/bumps off the clearance prism.
    for (const side of [-1, 1]) {
      const px = p.x + p.nx * side * (clearHalfW + 2.5);
      const pz = p.z + p.nz * side * (clearHalfW + 2.5);
      if (this._ribbonClear(px, pz, 1.2)) this._bump(px, pz, 1.1);
    }
  }

  /**
   * Drop any bridge mesh whose local AABB still invades the drive-through prism.
   * Axis-aligned refuse catches blocks; this catches rotated crown rubble.
   * @param {THREE.Group} g
      ...
      for (let xi = 0; xi < 2; xi++) {
        for (let yi = 0; yi < 2; yi++) {
          for (let zi = 0; zi < 2; zi++) {
            ...
            if (
              Math.abs(corner.x) < clearHalfW - 0.08 &&
              Math.abs(corner.z) < clearHalfD - 0.08 &&
              corner.y > 0.2 &&
              corner.y < openH - 0.12
            ) {
              hits += 1;
  ...
  _tunnelHill(along, dist, roadW) {
    ...
    const verge = roadW * 0.5 + 5;
    const peakAt = 22;
    const fallAt = 110;
    let lat = 0;
    if (dist <= verge) lat = 0;
    else if (dist < peakAt) lat = smoothstep((dist - verge) / Math.max(1, peakAt - verge));
    else if (dist < fallAt) lat = 1 - 0.12 * smoothstep((dist - peakAt) / (fallAt - peakAt));
    else lat = 0.88 * (1 - smoothstep((dist - fallAt) / 90));
    const jagged = 0.84 + 0.16 * Math.sin(along * 0.055 + dist * 0.04);
    return env * lat * 34 * jagged;
  }
  ...
  _addTunnelRun(start, end) {
    ...
    for (let i = start; i <= end; i += 1) {
      ...
      for (const side of [-1, 1]) {
        dummy.position.set(
          p.x + p.nx * side * (half + wallT * 0.42),
          p.y + wallH * 0.5 - 0.2,
          p.z + p.nz * side * (half + wallT * 0.42)
        );
        ...
        w += 1;
        this._bump(dummy.position.x, dummy.position.z, 1.15);
      }
  ...
  _addTunnelPortal(p, outward, rock, rockDark) {
    ...
    const postD = 4.2;
    for (const side of [-1, 1]) {
      const post = new THREE.Mesh(new THREE.BoxGeometry(postW, openH + 0.8, postD), rockDark);
      post.position.set(side * (half + postW * 0.55 + 0.35), (openH + 0.8) * 0.5, 0);
      post.castShadow = true;
      post.receiveShadow = false;
      g.add(post);
      const wing = new THREE.Mesh(new THREE.BoxGeometry(14, 24, 20), rock);
      wing.position.set(side * (half + 10), 11, outward * 5);
      wing.castShadow = true;
      wing.receiveShadow = false;
      g.add(wing);
      const buttress = new THREE.Mesh(new THREE.BoxGeometry(18, 28, 22), rockDark);
      buttress.position.set(side * (half + 20), 12.5, outward * 8);
    ...
    g.add(lintel);
    // Mountain mass over the hole — the road punches through, it does not sit in air.
    const cap = new THREE.Mesh(new THREE.BoxGeometry(p.width + 32, 14, 22), rock);
    cap.position.set(0, 15.2, outward * 5);
    cap.receiveShadow = false;
    g.add(cap);
    const peak = new THREE.Mesh(new THREE.BoxGeometry(p.width + 18, 10, 16), rockDark);
    peak.position.set(0, 24.5, outward * 7);
    peak.receiveShadow = false;
    g.add(peak);
    const shardGeo = cliffShardGeometry();
    const scatter = [
      [half + 6, 14, outward * 2, 7.5],
      [-(half + 7), 16, outward * 3, 8.2],
      [4, 22, outward * 6, 6.4],
      [-5, 20, outward * 8, 7.1],
      [half + 12, 9, outward * 9, 5.8],
      [-(half + 13), 8, outward * 10, 6.2],
    ];
    for (let i = 0; i < scatter.length; i++) {
      const s = scatter[i];
      const shard = new THREE.Mesh(shardGeo, i % 2 ? rockDark : rock);
      shard.position.set(s[0], s[1], s[2]);
      shard.scale.setScalar(s[3]);
  ...
  _addTunnelMountain(start, end, rock, rockDark) {
    ...
    for (let i = start; i <= end; i += 2) {
      ...
      for (const side of [-1, 1]) {
        ...
        const off = half + 16.5 + Math.sin(i * 0.7 + side) * 2.2;
        const hx = p.x + p.nx * side * off;
        const hz = p.z + p.nz * side * off;
        const gy = this._groundHeight(hx, hz, "desert");
        const h = 12 + Math.abs(Math.sin(i * 0.51 + side)) * 8;
        masses.push({
          c: chunk,
          x: hx,
          y: Math.max(p.y + 1.2, gy) + h * 0.42,
          z: hz,
          sx: 7 + (i % 5) * 0.6,
          sy: h,
          sz: 9,
  ...
  _addSpectators(rng, def) {
    ...
    const standOff = desert ? 6.8 : forest ? 7.2 : 5.6;
   ...
   * IDs are the feature's location, not the car's — otherwise a 42 m
   * look-ahead speaks the same corner three times as dist / 20 ticks.
   * @param {number} dist
   * @param {number} [look]
   */
  noteAt(dist, look = 42) {
    const authored = findAuthoredNote(this.id, dist, look);
    if (authored) return authoredToNote(authored);

    const far = Math.min(this.length - 1, dist + look);
    const mid = Math.min(this.length - 1, dist + look * 0.55);
    const a = this.sample(dist);
    const ahead = this.sample(mid);

    if (ahead.tunnel && !a.tunnel) {
      return {
        id: `tunnel-${(ahead.dist / 32) | 0}`,
        kind: "jump",
        dir: "AHEAD",
        severity: 2,
        text: "INTO TUNNEL",
        speech: "Into the tunnel",
      };
    }

    for (let d = dist + 8; d <= far; d += 6) {
      const p = this.sample(d);
      if (!p.jump) continue;
      return {
        id: `jump-${(p.dist / 32) | 0}`,
    ...
    for (let d = dist + 12; d <= far; d += 6) {
      const p = this.sample(d);
      if (!p.landmark) continue;
      const p0 = this.sample(Math.max(0, p.dist - 8));
      const p1 = this.sample(Math.min(this.length - 1, p.dist + 28));
      ...
      return {
        id: `hairpin-${dir}-${Math.round(p.dist / 140)}`,
        kind: "turn",
        dir,
        severity: 3,
        text: `HARD ${dir}`,
    ...
    let bestDh = 0;
    let bestAt = dist;
    for (let d = dist + 10; d <= far; d += 6) {
      const p0 = this.sample(Math.max(0, d - 16));
      const p1 = this.sample(d);
      let dh = p1.heading - p0.heading;
      while (dh > Math.PI) dh -= Math.PI * 2;
      while (dh < -Math.PI) dh += Math.PI * 2;
    ...
    if (bestDeg < 14) return null;

    const dir = bestDh > 0 ? "LEFT" : "RIGHT";
    const side = dir.toLowerCase();
    let severity = 1;
    let easy = "EASY";
    let speech = `Easy ${side}`;
    if (bestDeg > 125) {
      severity = 3;
      easy = "HARD";
      speech = `Hard ${side}`;
    ...
    const turn = `${easy ? easy + " " : ""}${dir}`.trim();
    return {
      id: `${dir}-${severity}-${Math.round(bestAt / 36)}`,
      kind: "turn",
      dir,
      severity,
      text: turn,
...
/**
 * Irregular rock shard — not a platonic solid. Used for cliff debris and
 * lakeside shore stones so they don't read as a bag of dodecahedra.
 */
